  ...
  filter(header, text) {
	...
	addItems(header, items) {
		items.forEach((d) => {
			...
			$("#" + header).append(`<div class="item ui-collapse search-content"><label class="checkbox" data-item-name="${d}"><h6 class="checklabel">${d.replace(/ /g, '&nbsp;')}</h6></label></div>`);
		});     
		...
		$("#" + header + " .checkbox").click(function(){ 
			...
			// FUNCTION PLACEHOLDER: redo this when it makes sense
			var numIndicators = $(".checkbox.checked").length;
			if (numIndicators == 0) {
				$(".output-UI:nth-child(1)").attr("src", "addons/placeholder");
				$(".output-UI:nth-child(2)").attr("src");
			}
			
			if (numIndicators == 1) {
				$(".output-UI:nth-child(1)").attr("src", "addons/country");
				$(".output-UI:nth-child(2)").attr("src");
			}			
			
			if (numIndicators > 1) {
				$(".output-UI:nth-child(1)").attr("src", "addons/country");
				$(".output-UI:nth-child(2)").attr("src", "addons/country");
			}
