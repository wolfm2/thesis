...
def plot(ax, ind):
  ...
  aggY = [0] * len(yearRange) # aggregate
  for cnt in areas:	
	...
	# make relative to 2009
	rel = y[0]
	for i,val in enumerate(y):	# add to em
		if val == 0:
			print(cnt, ind, "zero")
			continue
		aggY[i] += (rel / val) * 100
  ...
  for i,val in enumerate(aggY):  # average em
	aggY[i] = val/len(yearRange)

  ax.plot(x, aggY, "-|", label=cnt)
...
for i in inds:
	...
	plot_footer(fig, ax, i, 'AGG', iName)
